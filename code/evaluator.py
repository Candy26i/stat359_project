...
class ModelEvaluator:
    ...
    def evaluate(
        ...
        for batch_start in range(0, len(test_expressions), batch_size):
            ...
            for i, (expression, ground_truth, generated_text) in enumerate(
                ...
                full_ids = self.tokenizer.encode(generated_text, add_special_tokens=False)
                gen_len = max(0, len(full_ids) - len(prompt_ids))
                is_parseable = predicted_result is not None
                is_correct = is_parseable and predicted_result == ground_truth
    ...
    def _save_results(
        ...
        # Save metrics (exclude large 'samples' - saved in sample_outputs)
        metrics_to_save = {k: v for k, v in metrics.items() if k != 'samples'}
        ...
        with open(summary_path, 'w') as f:
            ...
            f.write("SAMPLE OUTPUTS (first 10 of %d):\n" % len(sample_outputs))
            f.write("-" * 60 + "\n")
